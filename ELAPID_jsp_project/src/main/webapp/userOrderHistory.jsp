...
<%
	...
	int contentCount = 1 ; // 1개 먼저 출력 하기 때문에 초기값 1 
	
	int allPageNum = (ohdtos.size() / contentsNumPerPage) + 1;  // 총 페이지 수
	int lastPageContentCount = ohdtos.size() % contentsNumPerPage ; // 마지막 페이지 컨텐츠 수 
	
	int pageNum = Integer.parseInt(request.getParameter("pageNum"));  // 현재 페이지 넘버 
			...
			<!-- 인덱스 0 만 따로 출력 끝 -->
				<div class="container">
					<div class="row">
						<div class="col-2" style="padding: 35px 0px 0px 0px;">
								<label class="form-check-label" for="flexCheckDefault"> 주문 번호 : <%=ohdtos.get(0).getUo_id()%><br></label>
						...
						<div class="col-4" align="left">
							...
							받는이 : <%= ohdtos.get(0).getUo_name() %><br>
							휴대폰 : <%=ohdtos.get(0).getUo_tel() %><br>
							주소 : <%=ohdtos.get(0).getUo_address()%><br>
							상세주소 : <%=ohdtos.get(0).getUo_specificaddress()%><br>
						...
						<div class="col -2" align="right">
							결제수단 : <%=ohdtos.get(0).getUo_paymentmethod() %><br>
							배송료 : <%=ohdtos.get(0).getUo_shippingfee() %>원<br>
							주문 금액 : <%=ohdtos.get(0).getUo_discountedamount() %>원<br>
							결제 금액 : <%=ohdtos.get(0).getUo_shippingfee() + ohdtos.get(0).getUo_discountedamount() %>원<br>
						</div>
					</div>
				</div>
				<hr width="1350">
				...
				<div class="row">
					...
					<div class="col-2" style="padding: 50px 0px 0px 0px;">
							<label class="form-check-label" for="flexCheckDefault"> 주문상세 번호 : <%=ohdtos.get(0).getUod_id()%><br></label><br>
							<label class="form-check-label" for="flexCheckDefault"> 상품 번호 : <%=ohdtos.get(0).getP_id()%><br></label>
					</div>
					<!-- 상품 이미지 -->
					<div class="col-2" align="left">
						<img src="<%=ohdtos.get(0).getImg_thum() %>" width="100" height="150">
					</div>
					<!-- 상품 정보 -->
					<div class="col-4" align="left" style="padding: 20px 0px 0px 0px;">
						상품 이름 : <%=ohdtos.get(0).getP_name() %><br>
						상품 분류 : <%=ohdtos.get(0).getCtg_sub() %><br>
						상품 색상 : <%=ohdtos.get(0).getP_colorname() %><br>
						상품 사이즈 : <%=ohdtos.get(0).getP_size() %><br>
						<br>
					</div>
					<!-- 상품 가격 -->
					<div class="col -2" align="right" style="padding: 40px 0px 0px 0px;">
						<p>상품 가격 : <%=ohdtos.get(0).getP_price() %>원</p>
					</div>
				</div>
			</div>
			<hr width="1350">
				...
				<div class="container">
					<div class="row">
						...
						<div class="col -2" align="right">
							...
							배송료 : <%=ohdtos.get(i).getUo_shippingfee() %>원<br>
							주문 금액 : <%=ohdtos.get(i).getUo_discountedamount() %>원<br>
							결제 금액 : <%=ohdtos.get(i).getUo_shippingfee() + ohdtos.get(i).getUo_discountedamount() %>원<br>
						</div>
					</div>
				</div>
				<hr width="1350">
		...
		%>
			<div class="container">
				<div class="row">
					...
					<!-- 상품 가격 -->
					<div class="col -2" align="right" style="padding: 40px 0px 0px 0px;">
						<p>상품 가격 : <%=ohdtos.get(i).getP_price() %>원</p>
					</div>
				</div>
			</div>
			<hr width="1350">
